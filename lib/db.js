...
module.exports.findMessages = function (max_results, callback) {
  onConnect(function (err, connection) {
    r.db(dbConfig['db']).table('messages').filter(
        r.row("fromType").eq('liveroom').or(r.row("fromType").eq('Group'))
      ).orderBy(r.desc('timestamp')).limit(max_results).run(connection, function(err, cursor) {
      if(err) {
...
// room_id : groupName  #直播吧20170704
module.exports.onNewMessages = function (room_id, callback) {
  onConnect(function (err, connection) {
    r.db(dbConfig['db']).table('messages')
    .filter(
      r.row('groupName').eq(room_id)
    )
    .filter(
      r.row('old_val').eq(null)
    )
    .orderBy(r.desc('timestamp')).limit(3)
...
module.exports.saveStatic = function (user, callback) {  
  onConnect(function (err, connection) {
    r.db(dbConfig.db).table('statics').insert(static).run(connection, function(err, result) {
      if(err) {
        logerror("[ERROR][%s][saveUser] %s:%s\n%s", connection['_id'], err.name, err.msg, err.message);
        callback(err);
      }
...
module.exports.saveRoom = function (user, callback) {  
  onConnect(function (err, connection) {
    r.db(dbConfig.db).table('rooms').insert(room).run(connection, function(err, result) {
      if(err) {
        logerror("[ERROR][%s][saveUser] %s:%s\n%s", connection['_id'], err.name, err.msg, err.message);
        callback(err);
      }
